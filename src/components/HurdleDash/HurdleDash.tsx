...
import { MARATHON_NO_ANSWER } from '../../utils/constants'
...
import { HurdleDashScene } from './HurdleDashScene'
...
export function HurdleDash() {
  ...
  const [jumpingPlayers, setJumpingPlayers] = useState<Set<PlayerId>>(new Set())
  const [trippedPlayers, setTrippedPlayers] = useState<Set<PlayerId>>(new Set())
  ...
  const resolveRound = useCallback(() => {
    ...
    // Hurdle Dash scoring: correct = +1 hurdle, wrong/no answer = 0
    for (const player of players) {
    ...
    // Track jumping and tripped players for visual effects
    const newJumpers = new Set<PlayerId>()
    const newTrippers = new Set<PlayerId>()
    for (const player of players) {
      const pid = player.id as PlayerId
      const result = playerResults.get(pid)
      if (result?.answer?.correct) {
        newJumpers.add(pid)
      } else if (result?.answer && !result.answer.correct) {
        newTrippers.add(pid)
      }
    }
    setJumpingPlayers(newJumpers)
    setTrippedPlayers(newTrippers)
    setTimeout(() => setJumpingPlayers(new Set()), 600)
    setTimeout(() => setTrippedPlayers(new Set()), 800)
    ...
    // Check for winner — first to clear all hurdles
  ...
  }, [players, currentProblem, trackLength, timePerQuestion, updatePosition, incrementStreak, resetStreak, incrementScore, setWinner, nextProblem, broadcastResult, soundEnabled, announceCorrect, announceWrong, broadcastSpectatorUpdate, resetLockIn])
  ...
  return (
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-indigo-950 to-gray-900 stars-bg screen-enter flex flex-col p-4 gap-4">
      ...
      {/* Pixel Art Hurdle Dash Scene */}
      <HurdleDashScene
        players={players}
        totalHurdles={trackLength}
        jumpingPlayers={jumpingPlayers}
        trippedPlayers={trippedPlayers}
      />
                ...
                return (
                  <div key={player.id} className="flex flex-col items-center gap-2">
                    ...
                    <div className={`font-pixel text-[10px] font-bold px-3 py-1.5 rounded ${
                      ...
                      {isCorrect ? 'CLEARED!' : result?.answer ? 'TRIPPED!' : 'NO ANSWER'}
                    </div>
                    <div className="font-pixel text-[9px] text-yellow-300">
                      {isCorrect ? '+1 hurdle' : '+0 hurdles'}
                    </div>
                  </div>
                )
              })}
      ...
      <div className="flex justify-between items-center">
        <div className="font-pixel text-[8px] text-white/30">HURDLE #{problemCount}</div>
